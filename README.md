...
**Other tools:** Express.js, Node.js, PM2, Nginx
...
- Input under 5 characters is rejected before any API call is made
- If an API is down or returns an error, the server catches it and shows a clear message to the user
- Empty results display a friendly "nothing found" message
- All network errors on the frontend are caught and shown as a readable error banner

---

## SSL Certificate

The app is secured with a **Let's Encrypt SSL certificate** configured on the load balancer. All traffic is encrypted over HTTPS (TLS 1.2/1.3) and HTTP requests are automatically redirected to HTTPS. The certificate is managed via Certbot and auto-renews every 90 days.
...
Traffic was verified by checking PM2 logs on both servers — requests alternate correctly between web-01 and web-02.
